...
$user = query("SELECT * FROM tb_donasi");
...
<script type="text/javascript" src="https://cdn.datatables.net/v/bs5/dt-1.12.0/r-2.3.0/datatables.min.js"></script>
   ...
   <script type="text/javascript">
    ...
    <title>Data Donatur | Humanity Care - Informasi Mengenasi Mitigasi dan Berita Seputar Bencana Alam</title>
</head>
<body> 
<div class="sidebar">
...
<table id="tab" class="table table-striped stripe row-border order-column  " cellspacing="3" width="100%">
        ...
        <tbody>
            ...
            <?php foreach ($user as $row ): ?>
                <tr>
            <td><?= $num;?></td>
            <td><a href="../path/hapusdonatur.php?id=<?= $row["id"];?>" onclick="return confirm('Apakakah data Donatur telah selesai di sumbangkan?');">Selesai</a></td>
            <td><?= $row["nama_donasi"];?></td>
            <td><?= $row["email_donasi"];?></td>
            <td><?= $row["alamat_donasi"];?></td>
            <td><?= $row["no_telp_donasi"];?></td>
            <td><?= $row["nominal_donasi"];?></td>
            <td><img src="img/barang/<?= $row["image_barang_donasi"];?>" width="100px"></td>
            <td><?= $row["keterangan_donasi"];?></td>
            <td><img src="img/bukti/<?= $row["image_bukti_donasi"];?>" width="100px"></td>
            </tr>
            <?php $num++ ?>
    <?php endforeach; ?>
        </tbody>
