...
<script type="text/javascript" src="https://cdn.datatables.net/v/bs5/dt-1.12.0/r-2.3.0/datatables.min.js"></script>
   ...
   <script type="text/javascript">
    ...
    <title>Data Postingan Berita | HumanityCare</title>
</head>
<body> 
<div class="sidebar">
...
<table id="tab" class="table table-striped stripe row-border order-column  " cellspacing="3" width="100%">
    ...
    <?php foreach ($berita as $row ): ?>
            <tr>
                <td><?= $num;?></td>
                <td>
                    <a href="editberita.php?id=<?= $row["id"];?>">Edit</a>
                    <a href="../path/hapusberita.php?id=<?= $row["id"];?>" onclick="return confirm('Yakin?');">Hapus</a>
                </td>
                <td><?= $row["judul_post_news"];?></td>
                <td><?= $row["kategori_post_news"];?></td>
                <td><?= $row["waktu_post_news"];?></td>
                <td><?= limit_text( $row["artikel_post_news"], 40)?></td>
                <td><img src="img/berita/<?= $row["img_post_news"];?>" width="150px"></td>
            </tr>
            <?php $num++ ?>
    <?php endforeach; ?>
        </tbody>
</table>
<a class="btn btn-primarys" href="berita.php" role="button">Tambah Postingan</a>
</div>
<!-- JS -->
